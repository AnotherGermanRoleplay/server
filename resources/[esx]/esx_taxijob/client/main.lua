...
local dataSend                  = false
local counter                   = 0
local distanceMoney             = 0
          ...
          if IsVehicleModel(vehicle, GetHashKey('Taxi')) then
            ...
            if IsPedSittingInVehicle(CurrentCustomer,  vehicle) then
              ...
              if CustomerEnteredVehicle then
                ...
                local targetDistance = CalculateTravelDistanceBetweenPoints(playerCoords.x,  playerCoords.y,  playerCoords.z,  TargetCoords.x,  TargetCoords.y,  TargetCoords.z)

                if counter < 5 then
                  Citizen.Trace(targetDistance)
                  if targetDistance < 100000 then
                    counter = counter + 1
                  end
                end

                if not dataSend and targetDistance < 50000 and counter == 5 then
                  dataSend = true
                  distanceMoney = targetDistance
                end
                ...
                if targetDistance <= 10.0 then
                  ...
                  TriggerServerEvent('esx_taxijob:getInfos', distanceMoney)
            ...
            DrawSub("Du musst in einem Taxi sitzen!", 5000)
