	...
	if DoesEntityExist(pP) then
		...
		Citizen.CreateThread(function()
			...
			while jT > 0 and not eJE do
				...
				if jT % 30 == 0 then
					TriggerEvent('chatMessage', 'SYSTEM', { 0, 0, 0 }, jT .." Sekunden noch eingesperrt.")
				end
				Citizen.Wait(500)
				...
				if D > 2 then -- distance#######################################################################################
					...
					if D > 4 then
						...
						TriggerEvent('chatMessage', 'JUDGE', { 0, 0, 0 }, "Die Zeit deiner Inhaftierung wurde erhöht!")
					end
				end
				jT = jT - 0.5
			end
			TriggerServerEvent('chatMessageEntered', "SYSTEM", { 0, 0, 0 }, GetPlayerName(PlayerId()) .." wurde freigelassen aus dem Gefängnis.")
