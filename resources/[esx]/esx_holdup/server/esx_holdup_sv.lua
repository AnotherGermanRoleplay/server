		...
		if rob == false then
			...
			if xPlayer.job.name ~= 'police' then
				...
				if cops > 1 then
					...
					--local wantedLevel = (5-cops)

					--TriggerClientEvent('esx:setWantedLevel', source, wantedLevel)
					...
					SetTimeout(300000, function()
						...
						if(robbers[savedSource])then
							...
							rob = false
							reward = store.reward
							TriggerClientEvent('esx_holdup:robberycomplete', savedSource, job, reward)
							if(xPlayer)then
				...
				TriggerClientEvent('esx:showNotification', source, "Polizisten können keine Läden ausrauben.")
			end
		else
