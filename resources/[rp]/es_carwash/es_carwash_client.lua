		...
		if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then 
			for i = 1, #vehicleWashStation do
				...
				if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), garageCoords2[1], garageCoords2[2], garageCoords2[3], true ) < 5 then
					es_carwash_DrawSubtitleTimed("Drücke [~g~ENTER~s~] um dein Auto zu waschen!")
...
AddEventHandler('es_carwash:success', function (price)
	...
	es_carwash_DrawNotification("Dein Fahrzeug wurde ~y~gesäubert~s~! ~r~-$" .. price .. "~s~")
end)

RegisterNetEvent('es_carwash:notenoughmoney')
AddEventHandler('es_carwash:notenoughmoney', function (moneyleft)
	es_carwash_DrawNotification("~h~~r~Du hast nicht genug Geld! $" .. moneyleft .. " fehlen dir!")
end)
...
AddEventHandler('es_carwash:free', function ()
	WashDecalsFromVehicle(GetVehiclePedIsUsing(GetPlayerPed(-1)), 1.0)
	SetVehicleDirtLevel(GetVehiclePedIsUsing(GetPlayerPed(-1)))
	es_carwash_DrawNotification("Dein Fahrzeug wurde ~y~gesäubert~s~, komplett kostenlos sogar!")
end)
