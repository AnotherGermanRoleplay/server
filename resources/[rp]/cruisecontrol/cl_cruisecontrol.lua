...
AddEventHandler('pv:setCruiseSpeed', function()
	if cruise == 0 and IsPedInAnyVehicle(GetPlayerPed(-1), false) then
		if GetEntitySpeedVector(GetVehiclePedIsIn(GetPlayerPed(-1), false), true)['y'] > 0 then
			...
			NotificationMessage("Tempomat: ~g~ AN~w~ - ~b~ " .. cruiseKm .." km/h")
			...
			Citizen.CreateThread(function()
				while cruise > 0 and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), false), -1) == GetPlayerPed(-1) do
					...
					if IsVehicleOnAllWheels(cruiseVeh) and  GetVehicleEngineHealth(cruiseVeh) > 0 and  GetEntitySpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false)) > (cruise - 2.0) then
						...
						cruise = 0
						NotificationMessage("Tempomat: ~r~ AUS")
						break
					end
					...
					if cruise > 44 then
						cruise = 0
						NotificationMessage("Tempomat: Kann nicht so hoch eingestellt werden!")
						break
					end
					Wait(200)
				end
