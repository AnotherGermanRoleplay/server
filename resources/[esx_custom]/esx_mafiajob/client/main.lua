...
function OpenArmoryMenu(station)
  ...
  if Config.EnableArmoryManagement then
    ...
    local elements = {
      ...
      { label = 'Objekt ablegen', value = 'put_stock' },
      { label = 'Objekt nehmen', value = 'get_stock' },
    }
...
function openPolice()
  if PlayerData.job ~= nil and PlayerData.job.name == 'mafia' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'police_actions') and (GetGameTimer() - GUI.Time) > 150 then
  ...
  if Config.EnableESXIdentity then
    ...
    ESX.TriggerServerCallback('esx_mafiajob:getOtherPlayerData', function(data)
      ...
      if data.sex ~= nil then
        ...
        sexLabel = 'Geschlecht : Unbekannt'
      end

      if data.dob ~= nil then
        dobLabel = 'Geb. : ' .. data.dob
      else
        dobLabel = 'Geb. : Unbekannt'
      end

      if data.height ~= nil then
        heightLabel = 'Größe : ' .. data.height
      else
        heightLabel = 'Größe : Unbekannt'
      end

      if data.name ~= nil then
        idLabel = 'ID : ' .. data.name
      else
        idLabel = 'ID : Unbekannt'
      end
  ...
  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine',
    {
      ...
      align = 'top-left',
      elements = {
        { label = 'STVO / Verkehrsdelikte', value = 0 },
        { label = 'BtMG / Drogen', value = 1 },
        { label = 'StGB / Wirtschaftskriminalität', value = 2 },
        { label = 'StGB / Waffendelikte', value = 3 },
        { label = 'StGB / Körperliche Integrität', value = 4 },
        { label = 'StGB / Umgang mit Beamten', value = 5 },
        { label = 'StGB / Sonstige Delikte', value = 6 }
      },
    },
    function(data, menu)
...
AddEventHandler('esx_mafiajob:hasEnteredEntityZone', function(entity)
  ...
  if GetEntityModel(entity) == GetHashKey('p_ld_stinger_s') and PlayerData.job.name ~= 'mafia' then
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsControlPressed(0, Keys['F6']) and PlayerData.job ~= nil and PlayerData.job.name == 'mafia' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'police_actions') and (GetGameTimer() - GUI.Time) > 150 then
      OpenPoliceActionsMenu()
      GUI.Time = GetGameTimer()
    end
  end
