	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			ESX.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 270.0, function(vehicle)
				...
				SetVehicleNumberPlateText(vehicle, "AGRP"..platenum)
...
function IsJobTrucker()
	if PlayerData ~= nil then
		...
		if PlayerData.second_job.name ~= nil and PlayerData.second_job.name == 'realgarbageman' then
			isJobTrucker = true
		end
		return isJobTrucker
	end
...
AddEventHandler('esx_realgarbageman:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_realgarbageman:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()    
    CurrentAction = nil
    CurrentActionMsg = ''
end)
...
function donnerlapaye()
	...
	if vievehicule >= 1 then
		if livraisonTotalPaye == 0 then
			...
			TriggerServerEvent("esx_realgarbageman:pay", amount)
	...
	while true do
		...
		if IsJobTrucker() then
			...
			if isInMarker and not hasAlreadyEnteredMarker then
				...
				TriggerEvent('esx_realgarbageman:hasEnteredMarker', currentZone)
			end

			if not isInMarker and hasAlreadyEnteredMarker then
				hasAlreadyEnteredMarker = false
				TriggerEvent('esx_realgarbageman:hasExitedMarker', lastZone)
			end
